...
def format_table_data(data, headers):
    ...
    # Исправлено: Корректное создание и возврат многострочной строки
    table_str = "\n".join(table_lines)
    return f"```\n{table_str}\n```"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Создаем inline-клавиатуру с меню
    keyboard = [
        [InlineKeyboardButton("📦 Поиск по заказу", callback_data="search_order")],
        [InlineKeyboardButton("📋 Поиск по изделию", callback_data="search_item")],
        [InlineKeyboardButton("🏷 Поиск упакованных изделий", callback_data="search_packaged")],
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "search_order":
        await query.edit_message_text(
            "📤 *Поиск по заказу*\n"
            "Введите номер заказа (например, `123`).",
            parse_mode='Markdown'
        )
    elif query.data == "search_item":
        await query.edit_message_text(
            "📋 *Поиск по изделию*\n"
            "Введите полный номер изделия (например, `123.45`).",
            parse_mode='Markdown'
        )
    elif query.data == "search_packaged":
        await query.edit_message_text(
            "🏷 *Поиск упакованных изделий*\n"
            "Введите номер заказа для поиска упакованных изделий (например, `123`).",
            parse_mode='Markdown'
        )
        ...
        help_text = (
            ...
            "🔎 *Поиск по заказу:*\n"
            "Отправьте номер заказа (например, `123`).\n"
            "Бот покажет все изделия из этого заказа по всем участкам.\n\n"
            "🔎 *Поиск по изделию:*\n"
            "Отправьте полный номер изделия (например, `123.45`).\n"
        ...
        # Добавляем кнопку "Назад" после помощи
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_text = (
        ...
        "❌ *Если данные не найдены:*\n"
        "Бот ответит: _\"Не запускали в работу\"_.\n\n"
        "⌨ Вы также можете использовать меню команд, отправив /start."
    )
    # Добавляем кнопку "Назад" после помощи
    keyboard = [[InlineKeyboardButton("⬅ Назад", callback_data="back_to_menu")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def packaged_items_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Проверка формата ввода: допускаем только цифры
    if not re.match(r'^\d+$', order_number):
        await update.message.reply_text(
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Проверка формата ввода: допускаем только цифры и точки
    if not re.match(r'^[\d.]+$', user_input):
        ...
        await update.message.reply_text(
            "❗ Пожалуйста, введите корректный номер заказа (например, `123`) или изделия (например, `123.45`).\n"
...
def main() -> None:
    ...
    application.add_handler(CallbackQueryHandler(button_handler, pattern="^(search_order|search_item|search_packaged|help)$"))
